(function () {
  ...
  function draw() {
    ...
    if (go) {
      ...
      if (nr && Math.floor(Date.now() / 250) % 2 === 0) {
        ...
        ctx.font = 'bold 16px sans-serif';
        ctx.textAlign = 'center';
        ctx.fillText('★ NEW RECORD! ★', 200, 165);
      }
      ...
      ctx.fillText('Score: ' + fn(s), 200, 260);
      ctx.fillText('Time: ' + (tm / 60).toFixed(2) + 's', 200, 290);

      if (gm === 6) {
        ctx.fillStyle = '#ff3366';
        ctx.font = 'bold 16px sans-serif';
        ctx.fillText('STAGE ' + storyStage + ' で撃沈...', 200, 320);
      }

      if (conn && conn.open && op) {
        ctx.fillStyle = '#00ffff';
        ctx.font = 'bold 14px sans-serif';
        ctx.fillText('対戦相手のスコア: ' + fn(op.s || 0) + (op.go ? ' (終了)' : ' (プレイ中)'), 200, 345);
        if (matchResult) {
          ctx.fillStyle = matchResult.indexOf('WIN') !== -1 ? '#ffff00' : (matchResult.indexOf('LOSE') !== -1 ? '#ff3333' : '#aaa');
          ctx.font = 'bold 20px sans-serif';
          ctx.fillText(matchResult, 200, 140);
        }
      }

      ctx.font = '13px sans-serif';
      ctx.fillStyle = '#888';
      if (gm === 0) ctx.fillText('自己ベスト: ' + fn(hi) + ' pts', 200, 320);
      if (gm === 1) ctx.fillText('自己ベスト: ' + fn(gn) + ' pts', 200, 320);
      if (gm === 2) ctx.fillText('自己ベスト: ' + (bt === 999999 ? '--.--s' : bt.toFixed(2) + 's'), 200, 320);
      if (gm === 3) ctx.fillText('自己ベスト: ' + (bt100 === 999999 ? '--.--s' : bt100.toFixed(2) + 's'), 200, 320);
      if (gm === 4) ctx.fillText('自己ベスト: ' + fn(cp) + ' pts', 200, 320);
      if (gm === 5) ctx.fillText('自己ベスト: ' + fn(tp) + ' pts', 200, 320);

      ctx.font = '16px sans-serif';
      ctx.fillStyle = '#00ff22';
      ctx.fillText('Rキーでリトライ', 200, 365);
      ctx.fillStyle = '#00f0ff';
      ctx.fillText('Mキーでモード選択に戻る', 200, 405);
    }
    ...
    if (win) {
      ...
      if (nr && Math.floor(Date.now() / 250) % 2 === 0) {
        ...
        ctx.font = 'bold 16px sans-serif';
        ctx.textAlign = 'center';
        ctx.fillText('★ NEW RECORD! ★', 200, 155);
      }

      ctx.fillStyle = '#ffcc00';
      ctx.font = 'bold 32px sans-serif';
      ctx.textAlign = 'center';
      ctx.fillText(gm === 6 ? 'TOWER CLEAR!' : (gm === 1 ? 'TIME UP!' : 'GAME CLEAR!'), 200, 200);

      ctx.fillStyle = '#fff';
      ctx.font = 'bold 22px sans-serif';
      if (gm === 6) {
        ctx.fillText('タワー完全脱出成功！', 200, 250);
        ctx.font = '14px sans-serif';
        ctx.fillStyle = '#00ffaa';
        ctx.fillText('クリアタイム: ' + (tm / 60).toFixed(2) + 's', 200, 280);
      } else if (gm === 1) {
        ctx.fillText('FINAL SCORE: ' + fn(s), 200, 250);
      } else {
        ctx.fillText('CLEAR TIME: ' + (tm / 60).toFixed(2) + 's', 200, 250);
      }

      if (conn && conn.open && op) {
        ctx.fillStyle = '#00ffff';
        ctx.font = 'bold 14px sans-serif';
        ctx.fillText('対戦相手のスコア: ' + fn(op.s || 0) + (op.go ? ' (終了)' : ' (プレイ中)'), 200, 305);
        if (matchResult) {
          ctx.fillStyle = matchResult.indexOf('WIN') !== -1 ? '#ffff00' : (matchResult.indexOf('LOSE') !== -1 ? '#ff3333' : '#aaa');
          ctx.font = 'bold 20px sans-serif';
          ctx.fillText(matchResult, 200, 135);
        }
      }

      ctx.font = '16px sans-serif';
      ctx.fillStyle = '#00ff22';
      ctx.fillText('Rキーで再挑戦', 200, 335);
      ctx.fillStyle = '#00f0ff';
      ctx.fillText('Mキーでモード選択へ', 200, 375);
    }
    ...
    if (cd > 0) {
      ...
      ctx.fillText(Math.ceil(cd / 60), 200, 300);
  ...
  function loop() {
    ...
    let meEnded = (go || win);
    let opEnded = (op && (op.go || op.win));

    if (conn && conn.open && meEnded && opEnded && !bpUpdated) {
      bpUpdated = true;
      let wf = false, lf = false;
      if (gm === 2 || gm === 3) {
        if (win && op.win) {
          if (tm < op.tm) wf = true;
          else if (tm > op.tm) lf = true;
        } else if (win) {
          wf = true;
        } else if (op.win) {
          lf = true;
      ...
      if (wf) {
        bp += 10;
        matchResult = 'WIN! +10 BP';
      } else if (lf) {
        bp = Math.max(0, bp - 2);
        matchResult = 'LOSE... -2 BP';
      } else {
        matchResult = 'DRAW (±0 BP)';
      }
      try { localStorage.setItem('jg_bp_v2', bp); } catch (e) {}
    }
  ...
  function destroy() {
    ...
    Object.keys(dpadActive).forEach(k => clearInterval(dpadActive[k].tid));
    if (peer) {
      peer.destroy();
      peer = null;
      conn = null;
    }
    if (dpadWrap) dpadWrap.remove();
    c.remove();
  }

  cb.addEventListener('click', () => {
    if (showOption) destroy();
  });
  ...
  cv.addEventListener('click', e => {
    ...
    if (netStatus === 'WAITING' || netStatus === 'CONNECTING' || netStatus === 'LOADING') {
      netStatus = '';
      if (peer) { peer.destroy(); peer = null; conn = null; }
      return;
    }

    if (ms && !ve && !showGacha && !showOption) {
      if (my >= 420 && my <= 455) {
        if (mx >= 25 && mx <= 130) ve = true;
        if (mx >= 145 && mx <= 250) { showGacha = true; gachaMenuState = 0; gachaSelIndex = 0; gachaTopMsg = ''; }
        if (mx >= 265 && mx <= 370) showOption = true;
      }
      if (my >= 465 && my <= 500) {
        if (mx >= 25 && mx <= 190) cr();
        if (mx >= 205 && mx <= 370) jr();
      ...
      if (mx >= 250 && mx <= 345 && my >= 138 && my <= 178) {
        isRightPos = !isRightPos;
        try { localStorage.setItem('jg_pos_right', isRightPos); } catch (ex) {}
        applyPos();
        return;
      }
      if (mx >= 140 && mx <= 260 && my >= 515 && my <= 553) {
        showOption = false;
      }
    } else if (ve) {
      if (my >= 480 && my <= 525) {
        if (vePage === 0 && mx >= 220 && mx <= 380) { vePage = 1; return; }
        if (vePage === 1 && mx >= 20 && mx <= 180) { vePage = 0; return; }
      }
      ve = false; vePage = 0;
    } else if (showGacha) {
      if (gachaMenuState === 0) {
        if (mx >= 35 && mx <= 365) {
          if (my >= 100 && my <= 160) { gachaSelIndex = 0; gachaMenuState = 1; }
          else if (my >= 175 && my <= 235) { gachaSelIndex = 1; gachaMenuState = 2; }
          else if (my >= 250 && my <= 310) { gachaSelIndex = 2; gachaTopMsg = '※期間限定ガチャは現在開催されていません'; }
        }
        if (mx >= 140 && mx <= 260 && my >= 515 && my <= 553) {
          gachaSelIndex = 3; showGacha = false;
        }
        return;
      }

      if (gachaMenuState === 1) {
        if (mx >= 15 && mx <= 110 && my >= 15 && my <= 45) { gachaMenuState = 0; return; }
        if (mx >= 295 && mx <= 375 && my >= 420 && my <= 565) { showGacha = false; ms = false; init(0); return; }
        if (mx >= 90 && mx <= 310 && my >= 190 && my <= 228) { drawGacha(); return; }
        if (my >= 245 && my <= 275) {
          if (mx >= 35 && mx <= 135) gachaTab = 0;
          if (mx >= 150 && mx <= 250) gachaTab = 1;
          if (mx >= 265 && mx <= 365) gachaTab = 2;
          return;
        }
        if (gachaTab === 0) {
          for (let i = 0; i < 10; i++) {
            ...
            if (Math.hypot(mx - cx, my - cy) <= 22) {
              if (ownedColors[i]) {
                curColorIdx = i;
                try { localStorage.setItem('jg_cur_color', curColorIdx); } catch (e) {}
              }
              return;
            }
          }
          ...
          for (let i = 0; i < 5; i++) {
            ...
            if (Math.hypot(mx - cx, my - cy) <= 22) {
              if (ownedPatterns[i]) {
                curPatternIdx = i;
                try { localStorage.setItem('jg_cur_pattern', curPatternIdx); } catch (e) {}
              }
              return;
            }
          }
          ...
          for (let i = 0; i < 5; i++) {
            ...
            if (Math.hypot(mx - cx, my - cy) <= 22) {
              if (ownedTrails[i]) {
                curTrailIdx = i;
                try { localStorage.setItem('jg_cur_trail', curTrailIdx); } catch (e) {}
              }
              return;
            }
          }
        }
        if (mx >= 140 && mx <= 260 && my >= 515 && my <= 553) { showGacha = false; }
      }

      if (gachaMenuState === 2) {
        if (mx >= 15 && mx <= 110 && my >= 15 && my <= 45) { gachaMenuState = 0; return; }
        if (mx >= 90 && mx <= 310 && my >= 190 && my <= 228) { drawUltraGacha(); return; }
        for (let i = 0; i < ultras.length; i++) {
          let cy = 230 + i * 68;
          if (mx >= 35 && mx <= 365 && my >= cy && my <= cy + 58) {
            if (ownedUltras[i]) {
              curUltraIdx = i;
              try { localStorage.setItem('jg_cur_ultra', curUltraIdx); } catch (e) {}
            }
            return;
          }
        }
  ...
  c.addEventListener('click', () => cv.focus());
  loop();
  setTimeout(() => cv.focus(), 80);
})();
